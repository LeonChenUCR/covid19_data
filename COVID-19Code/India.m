...
try
    indLocation = find(contains(tableRecovered.CountryRegion,'India')==1)
catch exception
    searchLoc = strfind(tableRecovered.CountryRegion,'India')
...
try
    indLocation = find(contains(tableDeaths.CountryRegion,'India')==1)
catch exception
    searchLoc = strfind(tableDeaths.CountryRegion,'India')
    indLocation = find([searchLoc{:}]==1)
end
Deaths = table2array(tableDeaths(indLocation,5:end-1));

try
    indLocation = find(contains(tableConfirmed.CountryRegion,'India')==1)
catch exception
    searchLoc = strfind(tableConfirmed.CountryRegion,'India')
    indLocation = find([searchLoc{:}]==1)
end
...
% confirmed cases
minNum= round(0.04*max(Confirmed));

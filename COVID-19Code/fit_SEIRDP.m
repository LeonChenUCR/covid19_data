function [alpha1,beta1,gamma1,Lambda1,Kappa1,varargout] = fit_SEIRDP(I,R,D,Npop,S0,E0,time1,time,guess,varargin)
...
fs = 1./dt;
tTarget = datenum(time(1)-time1(1))+round(datenum(time-time(1))*fs)/fs; % Number of days with one decimal 
...
[Coeff,~,residual,~,~,~,jacobian] = lsqcurvefit(@(para,t) modelFun1(para,t),...
    guess,tTarget(:)',input,zeros(1,numel(guess)),[2 2 2 1 2 1 2],options);
